...
describe('Авторизация (e2e)', () => {
  ...
  beforeAll(async () => {
    // Живой брокер не нужен: подменяем только клиент auth-микросервиса,
    // а глобальные пайп и фильтр остаются настоящими — их поведение
    // и проверяем.
    const moduleFixture = await Test.createTestingModule({
      imports: [AppModule],
    })
      .overrideProvider(RMQ_CLIENT.AUTH)
      .useValue(createFakeAuthClient(USER, PASSWORD))
      .compile();
