 ...
 * доступен как `/api/v1/auth/...`.
 *
 * Версия в префиксе — не наше решение, а контракт платформы инкубатора: ingress
 * заведён на путь `/api/v1` и отдаёт его бэкенду как есть, без rewrite. Запрос
 * на `/api/...` до пода вообще не доезжает — его отбивает nginx с 404.
 */
